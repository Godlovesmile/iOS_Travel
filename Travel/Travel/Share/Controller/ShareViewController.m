...
@interface ShareViewController ()<MJRefreshBaseViewDelegate>
...
- (void)setupUserData
{
    ...
    params[@"access_token"] = @"2.004uQS7G9PmRTEfb7e17f5e22gAH4D";
    params[@"uid"] = @"5885336631";
    
    // 2.发送请求
    [NetworkManager getWithURL:@"https://api.weibo.com/2/users/show.json" params:params success:^(id json) {
        // 字典转模型
...
- (void)loadMoreData
{
    ...
    // 2.发送请求
    [NetworkManager getWithURL:@"https://api.weibo.com/2/statuses/home_timeline.json" params:params success:^(id json) {
...
- (void)showNewStatusCount:(int)count
{
    ...
    if (count) {
        NSString *title = [NSString stringWithFormat:@"%d条新的分享", count];
        [btn setTitle:title forState:UIControlStateNormal];
    } else {
        [btn setTitle:@"没有新的分享" forState:UIControlStateNormal];
    }
...
- (void)setupNavBar
{
    ...
    // 右边按钮
    self.navigationItem.rightBarButtonItem = [UIBarButtonItem itemWithIcon:@"navigationbar_pop_highlighted" highIcon:nil target:self action:@selector(rightClick)];
...
- (void)rightClick {
    ...
    IWComposeViewController *ctrl = [[IWComposeViewController alloc] init];
...
{
    NSLog(@"self.statusFrames.count = %ld",self.statusFrames.count);
